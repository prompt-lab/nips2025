# 多卡训练：torchrun --nproc_per_node=8 test.py
import torch
...
def get_dataloader(dataset_name, batch_size=1024, shuffle=True, root='./data', download=True):
    ...
    num_workers = 0  # 增加数据加载线程数
    pin_memory = True  # 启用内存锁页，加速数据传输到GPU
...
class SumConv2d(nn.Module):
    ...
    def forward(self, x):
        return sum(conv(x) for conv in self.conv2_list)
...
class ResNet18SVD(nn.Module):
    ...
    def replace_conv_with_svd(self, module, rank, head_num):
        if isinstance(module, nn.Conv2d):
            ...
            conv1 = conv1.to(module.weight.device)
            conv2_list = [conv2.to(module.weight.device) for conv2 in conv2_list]
...
def train_distillation(teacher_model, student_model, train_loader, test_loader, optimizer, temp=7, alpha=0.3, epochs=1):
    student_model.train()
    teacher_model.eval()
    train_losses = []
    test_accuracies = []
...
def train_svd_model(model, rank, head_num, train_loader, test_loader, criterion, optimizer, init_method=None, epochs=1, teacher_model=None):
    model_svd = copy.deepcopy(model)
    # 处理DataParallel的情况
    if isinstance(model_svd, nn.DataParallel):
        model_svd.module.apply_svd(rank, head_num)
    else:
    ...
    if init_method == "distillation":
        ...
        train_losses, test_accuracies = train_distillation(teacher_model, model_svd, train_loader, test_loader, optimizer_student, epochs=1)
        return train_losses, test_accuracies
...
# 初始化模型并并行化
teacher_model = TeacherModel(num_classes=num_classes)
teacher_model = parallelize_model(teacher_model).to(device)

student_model = StudentModel(num_classes=num_classes)
student_model = parallelize_model(student_model).to(device)

model = ResNet18SVD(num_classes=num_classes)
model = parallelize_model(model).to(device)
...
train_losses_teacher, test_accuracies_teacher = train_model(teacher_model, train_loader, test_loader, criterion, optimizer_teacher, epochs=1)

print("Training Student Model (ResNet-18) with Distillation...")
train_losses_distill, test_accuracies_distill = train_distillation(teacher_model, student_model, train_loader, test_loader, optimizer_student, epochs=1)

print("Training Original ResNet-18...")
train_losses_original, test_accuracies_original = train_model(model, train_loader, test_loader, criterion, optimizer, epochs=1)
...
for rank in ranks:
    for head_num in head_nums:
        for init_method in init_methods:
            ...
            train_losses, test_accuracies = train_svd_model(model, rank, head_num, train_loader, test_loader, criterion, optimizer, init_method=init_method, epochs=1, teacher_model=teacher_model)
...
for i, (key, (_, test_accuracies)) in enumerate(results.items()):
    ...
    if isinstance(svd_model, nn.DataParallel):
        svd_model.module.apply_svd(int(rank), int(head_num))
    else:
        svd_model.apply_svd(int(rank), int(head_num))
    svd_params = count_parameters(svd_model)
    plt.plot(test_accuracies, label=f'SVD-ResNet-18 ({key}, {svd_params:,} params)', linestyle=linestyles[i % len(linestyles)], marker=markers[i % len(markers)])
