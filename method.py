...
class GatedSumConv2d(nn.Module):
    ...
    def forward(self, x):
        ...
        # 计算门控得分
        y = torch.mean(x, dim=(2, 3))  # 全局平均池化，形状 [batch, C_in]
        gating_scores = self.gate(y)  # 形状 [batch, head_num]
        gating_weights = F.softmax(gating_scores, dim=-1)  # 形状 [batch, head_num]
        # 计算所有分支的输出
        expert_outputs = torch.stack([conv(x) for conv in self.conv2_list], dim=-1)  # [batch, C_out, H, W, head_num]
        # 计算加权和
        gating_weights = gating_weights.view(batch_size, 1, 1, 1, self.head_num)  # 广播
        out = torch.sum(gating_weights * expert_outputs, dim=-1)  # [batch, C_out, H, W]
